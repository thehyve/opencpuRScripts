...
# This is an example of how the the UI definition functions can be used to
# produce an analysis
...
# for instance: fetchData("CELL-LINE", "Normalised ratios",
# "default_real_projection")
fetchData <- function(apiUrl, auth.token, study.name, concept.match = NULL,
                        concept.link = NULL, projection = NULL) {
  res <- getHighdimData(apiUrl, auth.token, study.name, concept.match,
                        concept.link, projection)
  res
}
...
generateArtefactsHeatmap <- function(data, type = c("png", "svg")) {
    ...
    heatmap.2(data, dendrogram = "column", Rowv = NA, scale = "none",
